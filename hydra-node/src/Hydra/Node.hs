...
handleChainTx :: HydraNode tx m -> ChainState tx -> OnChainTx tx -> m ()
handleChainTx HydraNode{eq} chainState onChainTx = putEvent eq $ OnChainEvent{chainState, onChainTx}
...
processEffect HydraNode{hn, oc, server, eq, env = Environment{party}} tracer e = do
  ...
  case e of
    ...
    OnChainEffect postChainTx ->
      -- TODO: extend & retrieve 'ChainState' from `OnChainEffect` (affects HeadLogic)
      postTx oc ChainState postChainTx
